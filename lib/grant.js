...
function usePasswordGrant (done) {
  ...
  return this.model.getUser(uname, pword, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
      return done(error('invalid_credentials', 'User credentials are invalid'));
    }
...
/**
 * Check for a scope and check model how to handle it
 *
 * @param {Function} done
 * @this OAuth
 */
function handleScope (done) {
    var scope = this.req.body.scope;
    ...
    var self = this;
    this.model.handleScope(scope, this.user, function(err, keys){
        if (err) done(error('server_error', false, err));

        self.scope = _.extend({}, keys);
        done();
    });
}
...
function sendResponse (done) {
  ...
  if (this.scope) response = _.extend(response, this.scope);
